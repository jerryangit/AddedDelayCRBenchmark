...
def main():
    ...
    try:
        ...
        while i <= totalVehicle:
            ...
            spwn = world.try_spawn_actor(bp, spwnLoc[spwnRand])

            if spwn is not None:
                actor_list.append(spwn)
                spwn.set_autopilot()
                print('created %s at %d' % (spwn.type_id,spwnRand))
                i += 1
                spwnRand = random.randint(1,4)
                destRand = random.randint(1,4)
            ...
            for actor in actor_list:
                ...
                if actor.get_location().distance(carla.Location(x=-150, y=-35, z=0.3)) > 38 and actor.get_location().distance(carla.Location(x=0, y=0, z=0)) > 5:
                    print(actor.id,' left the area at ', actor.get_location().x,actor.get_location().y,actor.get_location().z)
